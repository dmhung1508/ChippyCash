...
try {
    ...
    if (!isset($input['bank_id'])) {
        throw new Exception('Bank ID là bắt buộc');
    }
    
    $bankId = $input['bank_id'];
    
    // Lấy thông tin bank account
    $stmt = $conn->prepare("SELECT * FROM bank_accounts WHERE id = ? AND user_id = ?");
    ...
    // Giải mã password
    $bankPassword = base64_decode($bankAccount['bank_password']);
    ...
    // Cập nhật last_sync
    $updateStmt = $conn->prepare("UPDATE bank_accounts SET last_sync = NOW() WHERE id = ?");
    $updateStmt->execute([$bankId]);
