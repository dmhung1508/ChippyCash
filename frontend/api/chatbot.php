...
/**
 * Hàm tạo phản hồi của chatbot dựa trên tin nhắn của người dùng
 */
function generateChatbotResponse($conn, $user_id, $message) {
    ...
    if (strpos($message, 'tổng quan') !== false || strpos($message, 'tình hình') !== false || strpos($message, 'tài chính') !== false) {
        ...
        $response .= "- Tổng thu nhập: " . number_format($totalIncome, 0, ',', '.') . " ₫\n";
        $response .= "- Tổng chi tiêu: " . number_format($totalExpense, 0, ',', '.') . " ₫\n";
        $response .= "- Số dư hiện tại: " . number_format($balance, 0, ',', '.') . " ₫\n\n";
    ...
    if (strpos($message, 'xu hướng') !== false || strpos($message, 'trend') !== false) {
        ...
        foreach ($expenseTrend as $item) {
            ...
            $response .= "- Tháng $monthName: " . number_format($item['total'], 0, ',', '.') . " ₫\n";
        }
    ...
    if (strpos($message, 'tiết kiệm') !== false || strpos($message, 'tiền') !== false || strpos($message, 'kế hoạch') !== false) {
        $monthlyIncome = $totalIncome / max(1, getTransactionCount($conn, $user_id, 'income') / 12);
        $monthlyExpense = $totalExpense / max(1, getTransactionCount($conn, $user_id, 'expense') / 12);
        ...
        $response .= "- Thu nhập ước tính hàng tháng: " . number_format($monthlyIncome, 0, ',', '.') . " ₫\n";
        $response .= "- Chi tiêu ước tính hàng tháng: " . number_format($monthlyExpense, 0, ',', '.') . " ₫\n";
        $response .= "- Tiềm năng tiết kiệm hàng tháng: " . number_format($savingPotential, 0, ',', '.') . " ₫\n\n";
        
        $response .= "Kế hoạch tiết kiệm đề xuất:\n";
        $response .= "1. Quỹ khẩn cấp (50%): " . number_format($savingPotential * 0.5, 0, ',', '.') . " ₫/tháng\n";
        $response .= "2. Tiết kiệm dài hạn (30%): " . number_format($savingPotential * 0.3, 0, ',', '.') . " ₫/tháng\n";
        $response .= "3. Quỹ giải trí/cá nhân (20%): " . number_format($savingPotential * 0.2, 0, ',', '.') . " ₫/tháng\n\n";
        
        $response .= "Với kế hoạch này, sau 1 năm bạn sẽ tiết kiệm được khoảng " . number_format($savingPotential * 12, 0, ',', '.') . " ₫.";
    ...
    // Phản hồi mặc định nếu không nhận diện được ý định
...
function getTransactionCount($conn, $user_id, $type = null) {
